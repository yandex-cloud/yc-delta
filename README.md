...
Время жизни метаданных, сохраняемых в YDB, по умолчанию составляет 24 часа, и регулируется параметром Spark-сессии `spark.io.delta.storage.S3DynamoDBLogStore.ddb.ttl`, устанавливаемый в секундах. Документация Delta Lake не рекомендует устанавливать этот параметр в "чрезмерно малое значение" во избежание потери данных. Из практических соображений минимальное разумное значение составляет 1 час (3600 секунд), при этом фактическое время удаления устаревших метаданных зависит от периодичности и момента срабатывания скрипта очистки.
